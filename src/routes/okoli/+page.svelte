...
<svelte:head>
	...
	<meta
		name="description"
		content={`Co dělat v okolí ${cabin.location.city} — turistika, lyžování, cyklotrasy a památky v Jeseníkách.`}
	/>
</svelte:head>
		...
		<div class="grid grid-cols-1 gap-5 md:grid-cols-2">
			{#each trips as trip (trip.id)}
				...
				>
					<Icon
						icon={trip.icon}
						class="h-8 w-8 shrink-0 text-[var(--color-accent)]"
						aria-hidden="true"
					/>
					<div class="flex-1">
						<div class="flex items-baseline justify-between gap-4">
							<h2 class="font-serif text-xl font-normal text-[var(--color-foreground)]">
								{trip.title}
							</h2>
							<span
								class="inline-flex shrink-0 items-center gap-1 font-sans text-xs text-[var(--color-muted)]"
							>
								<Icon icon="tabler:map-pin" class="h-3.5 w-3.5" />
								{trip.distance}
							</span>
						</div>
						<p class="mt-2 font-sans text-sm leading-relaxed text-[var(--color-muted)]">
							{trip.description}
						</p>
						<span
